...
export class CalendarComponent  implements OnInit {
  ...
  events: CalendarEvent[] = [
    ...
    { 
      ...
      color: 'orange', 
      description: 'Fecha límite para entregar el diseño del flujo de onboarding.', 
      participants: ['Tú', 'Ana'] 
    },
    { 
      id: 3, 
  ...
  createEvent(): void {
    ...
    const newEvent: CalendarEvent = {
      ...
      participants: this.eventForm.participants.split(',').map(p => p.trim()).filter(p => p)
    };
  ...
  const baseClasses = 'calendar-day bg-white/5 p-2 flex flex-col cursor-pointer transition-colors duration-200';
