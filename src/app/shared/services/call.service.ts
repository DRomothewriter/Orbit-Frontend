...
export class CallService {
  ...
  localStream: Observable<MediaStream | null> = this.localStream$.asObservable();
            ...
            this.sendTransport.on('produce', async ({ kind, rtpParameters }: any, callback: any, errback: any) => {
              try {
                socket.emit(
                  ...
                  (response: ProduceResponse | { error: string }) => {
   ...
   */
  async produce(track: MediaStreamTrack, kind: 'audio' | 'video'): Promise<void> {
    try {
      ...
      console.log(`[Call] Producing ${kind}, producer ID: ${producer.id}`);
    } catch (error) {
      ...
      socket.emit(
        ...
        async (data: ConsumeResponse | { error: string }) => {
          ...
          try {
            ...
            console.log(`[Call] Consumer creado para ${kind}, track:`, consumer.track);

            // Reanudar el consumer
            socket.emit('resumeConsumer', { consumerId: consumer.id }, () => {
              console.log(`[Call] Consumer ${kind} reanudado para peer ${peerId}`);
            });
            ...
            if (!peer.stream) {
              ...
              console.log(`[Call] Nuevo stream creado para peer ${peerId}`);
              
              // Agregar listener para cuando se agreguen tracks
              peer.stream.onaddtrack = (event) => {
                console.log(`[Call] Track agregado al stream de peer ${peerId}:`, event.track.kind);
            ...
            console.log(`[Call] Track ${kind} agregado a peer ${peerId}. Total tracks:`, peer.stream.getTracks().length);
            ...
            console.log(`[Call] Consuming ${kind} from peer ${peerId}. Stream:`, peer.stream);
            resolve();
          } catch (error) {
            reject(error);
          }
